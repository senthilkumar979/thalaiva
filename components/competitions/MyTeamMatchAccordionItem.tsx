...
export const MyTeamMatchAccordionItem = ({
  competitionId,
  row,
}: MyTeamMatchAccordionItemProps) => {
  const { match } = row
  ...
  const sub = `${new Date(
    match.date,
  ).toLocaleDateString()} · ${formatVenueLabel(match.venue)}`
  const sorted = [...row.playersWithPoints].sort(
    (a, b) => b.captainMultiplied - a.captainMultiplied,
  )
            ...
            {sorted.map((r) => {
              ...
              return (
                <li key={r.player._id}>
                  {match.isScored ? (
                    <Link
                      href={`/competitions/${competitionId}/my-team?player=${encodeURIComponent(
                        r.player._id,
                      )}&match=${encodeURIComponent(match._id)}`}
